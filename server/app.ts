...
app.get("/api/temp-create-test-user", async (req, res) => {
  try {
    const email = "ceo@zieads.com";
    const password = "asikasikjos14";

    // 1. Delete user if already exists to ensure fresh creation with the new password
    ...
    // 2. Create the user
    console.log(`[Temp Test User] Creating user: ${email}`);
    ...
    console.log(`[Temp Test User] Created user with ID: ${userId}`);

    // 3. Upsert into user_plan
    ...
    // 4. Upsert into user_credits
    const { error: creditsError } = await supabaseAdmin
      .from("user_credits")
      .upsert({
        user_id: userId,
        ai_chat_daily_remaining: 999999,
        skill_run_monthly_remaining: 999999,
        lifetime_skill_runs: 0,
        lifetime_ai_messages_sent: 0
      });
    if (creditsError) throw creditsError;

    // 5. Upsert into v3_subscription_tiers
    ...
    if (tierError) {
      console.warn(`[Temp Test User] Warning upserting v3_subscription_tiers:`, tierError.message);
    }

    res.json({
      success: true,
      message: `Test account ${email} created successfully with Agency plan and unlimited credits!`,
      userId
    });
  } catch (err: any) {
